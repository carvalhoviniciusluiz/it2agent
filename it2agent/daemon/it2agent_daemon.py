#!/usr/bin/env python3
"""it2agent-daemon — Tier 1 iTerm2 Python API orchestration daemon (#26).
...
Feature-flag gate: like every it2agent capability, the daemon is off by
default. It starts only when ``agent.daemon`` is ON (checked via the #11
...
PROG = "it2agent-daemon"
FLAG_KEY = "agent.daemon"
...
def flag_enabled() -> bool:
    """Return True iff ``agent.daemon`` is ON.
...
def _build_logger(verbose: bool) -> logging.Logger:
    ...
    handler.setFormatter(
        logging.Formatter("%(asctime)s %(levelname)s agent.daemon: %(message)s")
    )
...
def _run_forever(logger: logging.Logger) -> None:
    ...
    async def main(connection):
        ...
        # agent.status_board; a no-op when that flag is OFF.
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog=PROG,
        description="it2agent Tier 1 daemon: session registry + agent ingest + idle.",
    )
    parser.add_argument(
        "--no-gate",
        action="store_true",
        help="bypass the agent.daemon feature-flag gate (local testing).",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def main(argv: list[str] | None = None) -> int:
    ...
    # it2agent spawn subcommand (Tier 1.2, #27): route BEFORE the daemon flag
    # gate — spawning a tab is core and always allowed (only identity tagging is
    # gated, on agent.status_board). See spawn_cli below.
...
#
# `it2agent-daemon spawn [opts] -- <command>` opens a new iTerm2 tab running an
...
# wrapper it2agent/spawn/it2agent-spawn (#10): use the SHELL path on stock
...
#
# Gate: identity tagging gates on agent.status_board (NOT agent.daemon)
# — the same flag it2agent-emit self-gates on. When it is OFF the tab still
# spawns; the plan's variable list is empty so no identity is stamped.
STATUS_BOARD_FLAG = "agent.status_board"


def status_board_enabled(no_gate: bool = False) -> bool:
    """Return True iff identity tagging should happen: --no-gate/IT2AGENT_FORCE
    force it on, otherwise the agent.status_board flag decides. Reuses the
    ...
    try:
        ...
        return it2agent_flag.is_enabled(STATUS_BOARD_FLAG)
