#!/usr/bin/env python3
"""Spawn-plan computation for the it2agent daemon (Tier 1.2, #27).
...
Gating: identity tagging gates on ``agent.status_board`` (the same flag the
...
just not tagged), exactly like ``it2agent-emit`` no-ops when the flag is OFF.
"""
...
# Lifecycle statuses the board understands (matches it2agent-emit / the shell
...
def build_identity_variables(
    *,
    ...
    (the ``agent.status_board`` gate is OFF) this returns an EMPTY list — the
    tab spawns untagged, mirroring how ``it2agent-emit`` no-ops when gated off.
    """
    if not tag_identity:
        return []
...
def build_spawn_plan(
    *,
    ...
    wrapper. ``tag_identity`` is the already-resolved ``agent.status_board``
