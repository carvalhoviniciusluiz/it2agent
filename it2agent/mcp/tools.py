#!/usr/bin/env python3
"""Pure tool registry + handlers for the it2agent MCP surface (#18).
...
JSON-RPC/stdio loop (``server`` / ``it2agent_mcp``) is the only impure layer;
...
Backing (see ``it2agent/docs/design.md``): it2agent's durable state lives in
...
GUIDE_URI = "it2agent://guide"


def read_guide() -> str:
    """Return the text of ``AGENT_GUIDE.md`` (the single source of truth).

    Reads the sibling doc resolved relative to the ``it2agent/`` root. Raises
...
# never imports iterm2: production wires a subprocess launcher (it2agent_mcp),
...
_register(
    ToolSpec(
        name="help",
        description=(
            "Return the it2agent agent capability guide (AGENT_GUIDE.md) — the "
